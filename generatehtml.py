...
body.theme-neon .fab-option{box-shadow:0 4px 20px rgba(0,0,0,.4)}
...
body.theme-neon .fab-main{box-shadow:0 4px 24px rgba(0,229,255,.4)}
...
body.theme-neon .stat-card.sc-total::before{background:#bf00ff}
body.theme-neon .stat-card.sc-pending::before{background:#ffaa00}
body.theme-neon .stat-card.sc-completed::before{background:#00ff9d}
body.theme-neon .stat-card.sc-missed::before{background:#ff3366}
.stat-icon{font-size:20px;line-height:1}
.stat-num{font-family:'Fraunces',serif;font-size:28px;color:var(--text);line-height:1;font-weight:700}
...
body.theme-neon .badge-next{background:rgba(0,229,255,.15);color:#00e5ff}
body.theme-neon .badge-done{background:rgba(0,255,157,.12);color:#00ff9d}
...
.dash-greeting-name{font-family:'Fraunces',serif;font-size:20px;font-weight:700;color:var(--text)}
...
.dash-cal-month{
  font-family:'Fraunces',serif;font-size:13px;font-weight:700;color:var(--text);
}
.dash-cal-nav{
...
body.theme-neon  .dash-cal-cell.has-rem{background:rgba(0,229,255,.12);color:var(--accent)}
...
body.theme-neon  .dash-cal-cell.has-rem:hover{background:rgba(0,229,255,.22)}
.dash-cal-cell.is-today{
  background:var(--red);color:#fff!important;font-weight:700;
}
body.theme-neon .dash-cal-cell.is-today{background:#ff3366}
...
body.theme-neon .dash-tasks-count.open{background:rgba(0,229,255,.1);color:#00e5ff}
...
.notes-page-wrap{display:flex;flex-direction:row;height:calc(100vh - 58px);overflow:hidden}
...
.notes-folders-title{
  font-family:'Fraunces',serif;font-size:14px;font-weight:700;color:var(--text)
}
...
body.theme-neon  .notes-folder-item.active{background:rgba(0,229,255,.08);color:var(--accent)}
...
.notes-list-hdr-top{display:flex;align-items:center;justify-content:space-between}
...
.full-cal-title{
  font-family:'Fraunces',serif;font-size:18px;font-weight:700;color:var(--text);
...
.notes-md-preview h1{font-family:'Fraunces',serif;font-size:22px;font-weight:700;color:var(--text);margin:18px 0 8px;border-bottom:2px solid var(--border);padding-bottom:6px}
.notes-md-preview h2{font-family:'Fraunces',serif;font-size:18px;font-weight:700;color:var(--text);margin:16px 0 6px}
.notes-md-preview h3{font-family:'Fraunces',serif;font-size:15px;font-weight:700;color:var(--accent);margin:12px 0 4px;text-transform:uppercase;letter-spacing:.5px}
...
body.theme-neon .rem-tile-today{background:#0066cc}
body.theme-neon .rem-tile-scheduled{background:#cc2222}
body.theme-neon .rem-tile-all{background:#334}
body.theme-neon .rem-tile-completed{background:#007744}
...
.rem-lists-title{font-family:'Fraunces',serif;font-size:14px;font-weight:700;color:var(--text)}
.rem-new-list-btn{
...
body.theme-neon  .rem-list-item.active{background:rgba(0,229,255,.08);color:var(--accent)}
...
.frow input,.frow textarea,.frow select{
  ...
  font-family:'EB Garamond',serif;font-size:15px;outline:none;transition:border-color 0.2s
}
...
.fin-group-name{font-family:'Fraunces',serif;font-size:13px;font-weight:700;color:var(--text);transition:color 0.15s}
...
body.theme-neon .fin-vtbtn.active{color:#080c14}
...
@media(max-width:860px){
  ...
  /* Notes: hide folders panel on tablet, keep list+editor */
...
@media(max-width:400px){
  ...
  .notes-list-panel{height:45vh}
  .notes-editor-panel{height:55vh}
}
...
@media(max-width:640px){
  /* ── NOTES PAGE ── */
  .notes-page-wrap{
    flex-direction:row;height:calc(100vh - 58px);
    overflow:hidden;position:relative
  }
  /* All 3 panels become full-width slides */
  .notes-folders-panel,
  .notes-list-panel,
  .notes-editor-panel{
  ...
  /* Default: folders visible, list and editor off-screen right */
  ...
  /* State: showing list panel */
  .notes-page-wrap.show-list .notes-folders-panel{transform:translateX(-100%)}
  .notes-page-wrap.show-list .notes-list-panel{transform:translateX(0)}
  .notes-page-wrap.show-list .notes-editor-panel{transform:translateX(100%)}
  /* State: showing editor panel */
  .notes-page-wrap.show-editor .notes-folders-panel{transform:translateX(-100%)}
  .notes-page-wrap.show-editor .notes-list-panel{transform:translateX(-100%)}
  .notes-page-wrap.show-editor .notes-editor-panel{transform:translateX(0);z-index:4}
  /* Mobile back bar */
  .notes-mobile-back{
    display:flex;align-items:center;gap:8px;
  ...
  /* Make folders panel scrollable, fix heights */
...
<div class="main">
  ...
  <div id="page-notes" style="display:none;flex-direction:row;height:calc(100vh - 58px)">
...
<div class="settings-modal">
  ...
  <!-- ENCRYPTION -->
  <div class="settings-section-title" style="margin-top:24px">🔒 Encryption</div>
  <p style="font-size:12px;color:var(--muted);margin-bottom:14px;line-height:1.6">
    Encrypt your <code style="background:var(--s2);padding:1px 5px;border-radius:4px">notes.json</code> on GitHub with AES-256-GCM.
    The key is stored <strong>only in this browser</strong>. Export it to back up!
  </p>
  <div id="enc-status" style="display:flex;align-items:center;gap:10px;margin-bottom:14px">
    <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px;color:var(--text)">
      <input type="checkbox" id="cfg-enc-toggle" onchange="toggleEncryption(this.checked)" style="accent-color:var(--accent);width:16px;height:16px">
      Enable encryption
    </label>
    <span id="enc-badge" style="font-size:11px;padding:2px 10px;border-radius:20px;font-weight:600;display:none"></span>
  </div>
  <div id="enc-key-section" style="display:none">
    <div class="frow">
      <label>Encryption Key (base64) — paste or edit directly</label>
      <div style="display:flex;gap:6px">
        <input id="cfg-enc-key" type="password" placeholder="Paste your key here or generate one..." style="flex:1;font-family:monospace;font-size:12px">
        <button class="btn-ghost" onclick="toggleKeyVisibility()" title="Show/Hide" style="padding:7px 10px;font-size:14px">👁</button>
        <button class="btn-ghost" onclick="copyEncKey()" title="Copy" style="padding:7px 10px;font-size:14px">📋</button>
        <button class="btn" onclick="saveEncKeyFromInput()" title="Save Key" style="padding:7px 12px;font-size:12px">💾 Save</button>
      </div>
    </div>
    <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px">
      <button class="btn-ghost" onclick="generateNewEncKey()" style="font-size:12px">🔑 Generate New Key</button>
      <button class="btn-ghost" onclick="document.getElementById('enc-key-import').click()" style="font-size:12px">📥 Import File</button>
      <button class="btn-ghost" onclick="exportEncKey()" style="font-size:12px">📤 Export File</button>
      <input type="file" id="enc-key-import" accept=".key,.txt" style="display:none" onchange="importEncKey(event)">
    </div>
    <p style="font-size:11px;color:var(--red);line-height:1.5;margin-bottom:4px">
      ⚠️ If you lose this key, your encrypted notes <strong>cannot</strong> be recovered. Always keep a backup!
    </p>
  </div>
...
function getEncEnabled(){ return localStorage.getItem('enc_enabled')==='true'; }
function getEncKeyB64(){ return localStorage.getItem('enc_key')||''; }
...
async function encryptData(plaintext){
  const b64 = getEncKeyB64();
  if(!b64) throw new Error('No encryption key');
  const key = await importCryptoKey(b64);
...
async function decryptData(b64Combined){
  const b64Key = getEncKeyB64();
  if(!b64Key) throw new Error('No encryption key — cannot decrypt');
  const key = await importCryptoKey(b64Key);
...
function isEncryptedPayload(obj){
  return obj && typeof obj==='object' && obj.__encrypted===true && typeof obj.__data==='string';
}

async function generateNewEncKey(){
  if(getEncKeyB64()){
    if(!confirm('This will generate a NEW key. Your existing encrypted data will need to be re-encrypted. Continue?')) return;
  }
  const key = await crypto.subtle.generateKey({name:'AES-GCM',length:256}, true, ['encrypt','decrypt']);
  const raw = await crypto.subtle.exportKey('raw', key);
  const b64 = btoa(String.fromCharCode(...new Uint8Array(raw)));
  localStorage.setItem('enc_key', b64);
  localStorage.setItem('enc_enabled','true');
  refreshEncUI();
  toast('New encryption key generated ✓','success');
  // If data is loaded, re-save with new encryption
  if(dataLoaded && getConfig().token){
    await saveToGitHub();
    toast('Data re-encrypted & saved ✓','success');
  }
}

function toggleEncryption(checked){
  if(checked){
    if(!getEncKeyB64()){
      generateNewEncKey();
    } else {
      ...
      if(dataLoaded && getConfig().token) saveToGitHub();
    }
  } else {
    if(confirm('Disable encryption? Your notes.json on GitHub will be saved as plain text.')){
      localStorage.setItem('enc_enabled','false');
      refreshEncUI();
      toast('Encryption disabled','success');
      if(dataLoaded && getConfig().token) saveToGitHub();
    } else {
      document.getElementById('cfg-enc-toggle').checked = true;
    }
  }
}

function refreshEncUI(){
  const enabled = getEncEnabled();
  const hasKey = !!getEncKeyB64();
  const toggle = document.getElementById('cfg-enc-toggle');
  const badge = document.getElementById('enc-badge');
  const keySection = document.getElementById('enc-key-section');
  const keyInput = document.getElementById('cfg-enc-key');
  if(toggle) toggle.checked = enabled;
  if(badge){
    if(enabled && hasKey){
      badge.style.display='inline';
      badge.textContent='Active';
      badge.style.background='rgba(42,122,64,.15)';
      badge.style.color='var(--green)';
    } else if(hasKey && !enabled){
      badge.style.display='inline';
      badge.textContent='Paused';
      badge.style.background='rgba(200,160,80,.15)';
      badge.style.color='var(--accent)';
    } else {
      badge.style.display='none';
    }
  }
  if(keySection) keySection.style.display = (enabled||hasKey) ? 'block' : 'none';
  if(keyInput) keyInput.value = hasKey ? getEncKeyB64() : '';
}

function toggleKeyVisibility(){
  const inp = document.getElementById('cfg-enc-key');
  if(!inp) return;
  inp.type = inp.type==='password' ? 'text' : 'password';
}

function copyEncKey(){
  const k = getEncKeyB64();
  if(!k){toast('No key to copy','error');return;}
  navigator.clipboard.writeText(k).then(()=>toast('Key copied ✓','success')).catch(()=>{
    // Fallback
    const ta=document.createElement('textarea');ta.value=k;document.body.appendChild(ta);ta.select();document.execCommand('copy');ta.remove();
    toast('Key copied ✓','success');
  });
}

function exportEncKey(){
  const k = getEncKeyB64();
  if(!k){toast('No key to export','error');return;}
  const blob = new Blob([k],{type:'text/plain'});
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = 'mynotes-encryption.key';
  a.click();
  URL.revokeObjectURL(a.href);
  toast('Key exported ✓','success');
}

function importEncKey(e){
  const file = e.target.files[0];
  if(!file) return;
  const reader = new FileReader();
  reader.onload = function(){
    const k = reader.result.trim();
    // Validate: should be base64, 44 chars for 256-bit key
    try{ atob(k); } catch(_){ toast('Invalid key file','error'); return; }
    if(confirm('Import this key? If you have existing encrypted data, make sure this is the correct key.')){
...
function saveEncKeyFromInput(){
  const inp = document.getElementById('cfg-enc-key');
  if(!inp) return;
  const k = inp.value.trim();
  if(!k){ toast('Key field is empty','error'); return; }
  // Validate base64
  try{ atob(k); }catch(_){ toast('Invalid key — must be valid base64','error'); return; }
  if(getEncKeyB64() && k !== getEncKeyB64()){
    if(!confirm('Replace existing key? Make sure this is correct — wrong key = unreadable data.')) return;
  }
  localStorage.setItem('enc_key', k);
  localStorage.setItem('enc_enabled','true');
  refreshEncUI();
  toast('Key saved ✓','success');
  // Re-encrypt data if loaded
  if(dataLoaded && getConfig().token) saveToGitHub();
}
...
async function loadFromGitHub(){
  ...
  try{
    ...
    fileSHA=j.sha;
    let rawContent = JSON.parse(decodeURIComponent(escape(atob(j.content.replace(/\n/g,'')))));
    // Handle encrypted payload
    if(isEncryptedPayload(rawContent)){
      if(!getEncKeyB64()){
        toast('Data is encrypted but no key found — import your key in Settings','error');
        dataLoaded=true; initSticky(); renderAll();
        setSyncing(false,'Encrypted');
        openSettings();
        return;
      }
      try{
        const decrypted = await decryptData(rawContent.__data);
        rawContent = JSON.parse(decrypted);
      }catch(e){
        toast('Decryption failed — wrong key?','error');
        dataLoaded=true; initSticky(); renderAll();
        setSyncing(false,'Decrypt Error');
        openSettings();
        return;
      }
    }
    DATA=rawContent;
    dataLoaded=true; // data is now safely loaded — saves are now allowed
    let needsRepair = false;

    // ── CRITICAL: Preserve user data arrays before any repair logic ──
    const _savedNotes     = Array.isArray(DATA.notes)      ? [...DATA.notes]      : [];
    const _savedReminders = Array.isArray(DATA.reminders)  ? [...DATA.reminders]  : [];
    const _savedFinance   = Array.isArray(DATA.finance)    ? [...DATA.finance]    : [];
    const _savedTrades    = Array.isArray(DATA.trades)     ? [...DATA.trades]     : [];
    const _savedTasknotes = Array.isArray(DATA.tasknotes)  ? [...DATA.tasknotes]  : [];

    // Snapshot before repair to detect genuine changes
    ...
    // ── EMOJI SANITIZER ──────────────────────────────────────────────────────
    // Detects garbled multi-byte strings in icon/color/emoji fields and resets them.
    // Happens when old saves used plain btoa() without encodeURIComponent wrapper.
    ...
    if(!Array.isArray(DATA.note_folders)||!DATA.note_folders.length||
      ...
      DATA.note_folders = DATA.note_folders.map(f=>({
        ...f,
        icon: safeIcon(f.icon,'📁'),
        name: safeName(f.name,'Folder')
      }));
    }
    ...
    if(!Array.isArray(DATA.rem_lists)||!DATA.rem_lists.length||
      ...
      DATA.rem_lists = DATA.rem_lists.map(l=>({
        ...l,
        icon: safeIcon(l.icon,'🔵'),
        name: safeName(l.name,'List')
      }));
    }

    // Fix stickies — bg, colorId
    if(Array.isArray(DATA.stickies)){
      DATA.stickies = DATA.stickies.map(s=>({
        ...s,
        text: isValidStr(s.text) ? s.text : '',
        bg:   isValidStr(s.bg)   ? s.bg   : '#fde68a'
      }));
    }
    // ─────────────────────────────────────────────────────────────────────────
    // ── RESTORE: Always keep the real user data safe ──
    // The sanitizer only fixes icon/emoji fields. Core data arrays are never wiped.
    if(_savedNotes.length     > 0) DATA.notes     = _savedNotes;
    if(_savedReminders.length > 0) DATA.reminders = _savedReminders;
    if(_savedFinance.length   > 0) DATA.finance   = _savedFinance;
    if(_savedTrades.length    > 0) DATA.trades    = _savedTrades;
    if(_savedTasknotes.length > 0) DATA.tasknotes = _savedTasknotes;
    ...
    renderTaskNotes();
    renderFinance();
    initSticky(); // re-init with synced DATA

    if(needsRepair){
      // Double-check: only save if data actually changed from what was on GitHub
      const _afterRepair = JSON.stringify(DATA);
      if(_afterRepair !== _beforeRepair){
        await saveToGitHub();
    ...
    // On first use notes.json won't exist (404), or network may fail.
    // Still mark dataLoaded so the user can start using the app and save new data.
    dataLoaded=true;
    initSticky();
    renderAll();
...
async function saveToGitHub(){
  ...
  try{
    // Prepare payload — encrypt if enabled
    let payload;
    if(getEncEnabled() && getEncKeyB64()){
      setSyncing(true,'Encrypting...');
      const plainJson = JSON.stringify(DATA,null,2);
      const encB64 = await encryptData(plainJson);
      payload = JSON.stringify({__encrypted:true, __data:encB64});
    } else {
    ...
    if(!res.ok){
      ...
      if(res.status===409||res.status===422||e.message?.includes('does not match')){
        ...
        toast(getEncEnabled()?'Encrypted & saved ✓':'Saved ✓','success');
        setSyncing(false,'Synced');
        return true;
      }
...
async function refreshSHA(){
  ...
  try{
    ...
    if(res.ok){
      const j=await res.json();
      fileSHA=j.sha;
      // Merge remote data — only fall back to remote if our local copy is genuinely empty
      // Use .length check (not ||) because [] is truthy but still means "no data"
      let remote=JSON.parse(decodeURIComponent(escape(atob(j.content.replace(/\n/g,'')))));
      // Decrypt if needed
      if(isEncryptedPayload(remote)){
        try{
          const dec = await decryptData(remote.__data);
          remote = JSON.parse(dec);
        }catch(e){ console.warn('refreshSHA decrypt failed',e); return; }
      }
      if(!DATA.notes.length)        DATA.notes        = remote.notes        || [];
      if(!DATA.reminders.length)    DATA.reminders    = remote.reminders    || [];
      if(!DATA.trades.length)       DATA.trades       = remote.trades       || [];
      if(!DATA.routines.length)     DATA.routines     = remote.routines     || [];
      if(!DATA.routine_logs.length) DATA.routine_logs = remote.routine_logs || [];
      if(!DATA.tasknotes.length)    DATA.tasknotes    = remote.tasknotes    || [];
      if(!DATA.finance.length)      DATA.finance      = remote.finance      || [];
      if(!DATA.note_folders.length) DATA.note_folders = remote.note_folders || [{id:'personal',name:'Personal',icon:'👤'},{id:'official',name:'Official',icon:'💼'}];
      if(!DATA.rem_lists.length)    DATA.rem_lists    = remote.rem_lists    || [{id:'personal',name:'Personal',icon:'🔵'},{id:'official',name:'Official',icon:'🔴'}];
      if(!DATA.stickies.length)     DATA.stickies     = remote.stickies     || [];
      if(!DATA.archived.length)     DATA.archived     = remote.archived     || [];
    }
...
function setSyncing(on,label){
  ...
  const encIcon = (getEncEnabled() && getEncKeyB64()) ? '🔒 ' : '';
  document.getElementById('stext').textContent=encIcon+(label||'Synced');
}
...
function notesMobileShow(panel){
  if(!isMobile()) return;
  const wrap = document.querySelector('.notes-page-wrap');
  if(!wrap) return;
  wrap.classList.remove('show-list','show-editor');
  if(panel==='list')   wrap.classList.add('show-list');
  if(panel==='editor') wrap.classList.add('show-editor');
}
...
function notesMobileBack(to){
  ...
  if(!wrap) return;
  wrap.classList.remove('show-list','show-editor');
  if(to==='list') wrap.classList.add('show-list');
...
function showPage(page, btn){
  const pages = ['dashboard','notes','reminders','sticky','journal','routine','tasknotes','finance'];
  const displayMap = {dashboard:'',notes:'flex',reminders:'flex',sticky:'flex',journal:'flex',routine:'flex',tasknotes:'flex',finance:'flex'};
  pages.forEach(p=>{
  ...
  // 5. Context-aware actions
  ['dashboard','notes','reminders','sticky','journal','routine','tasknotes','finance'].forEach(p=>{
...
function renderTaskNotes(){
  ...
  function renderItem(n){
    ...
    return `<div class="tan-item${n.done?' is-done':''}" id="tan-item-${n.id}" style="display:flex;flex-direction:row">
...
function toggleFinRepay(id){
  ...
  document.getElementById('fin-pay-date').value=new Date().toISOString().slice(0,10);
...
function renderDashUpcomingList(remDates, todayStr, reminders){
  ...
  if(_dashCalSelDay){
    ...
    list = reminders
      .filter(r=>!r.sent&&r.due&&r.due.slice(0,10)>=todayStr)
...
function updateDashboardWidgets(){
  ...
  if(greetText){
    const userName = (getConfig().user||'').replace(/[^a-zA-Z]/g,'') || '';
...
const TEMPLATES = {
  daily: {
    title: '📅 Daily Log — '+new Date().toISOString().slice(0,10),
    body: `## Morning
- 
...
`
  ...
  trading: {
    title: '📈 Trading Plan — '+new Date().toISOString().slice(0,10),
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  const c=getConfig();
  if(!c.token){
    // No GitHub token — allow saves so new users can start adding data
    dataLoaded=true;
    initSticky(); // safe: no token means saveToGitHub() won't fire
    openSettings();
    renderAll();
  }
  else loadFromGitHub();
