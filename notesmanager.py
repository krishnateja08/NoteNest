...
.stat-label{font-size:12px;color:var(--text2);margin-top:3px;font-weight:700;letter-spacing:0.3px}
...
.rem-checklist-hdr{
  padding:16px 28px 12px;border-bottom:1px solid var(--border);flex-shrink:0;
  display:flex;align-items:center;justify-content:space-between
}
.rem-checklist-title{
...
.rem-checklist-body{flex:1;overflow-y:auto;padding:8px 28px 20px}
.rem-item-row{
...
.fin-list-thead{
  display:grid;
  grid-template-columns:3px 36px 1fr 90px 110px 100px 90px 200px;
...
.fin-lrow-main{flex:1;min-width:0;padding:0 10px}
.fin-lrow-person{font-size:13px;font-weight:700;color:var(--text)}
.fin-lrow-note{font-size:11px;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:200px}
...
<div class="main">
  ...
  <div id="page-dashboard">
    <!-- Notification permission prompt (shown if notifications not granted) -->
    <div id="notif-prompt" class="notif-prompt" style="display:none;margin-top:12px">
...
function _renderRemChecklist(){
  ...
  if(done.length){
    html += `<div class="rem-completed-section">
      <div class="rem-completed-toggle" onclick="toggleRemCompleted(this)">
...
function updateDashboardWidgets(){
  ...
  const wEl = document.getElementById('w-wordcount');
  if(wEl) wEl.textContent = words.toLocaleString();

  // Streak — count consecutive days with at least one note edited
  const today = new Date().toISOString().slice(0,10);
  const days = new Set(notes.map(n=>(n.updated||n.created||'').slice(0,10)).filter(Boolean));
  ...
  const sEl = document.getElementById('w-streak');
  const sSub = document.getElementById('w-streak-sub');
  if(sEl) sEl.textContent = streak+' day'+(streak!==1?'s':'');
  if(sSub) sSub.textContent = streak>0?'Keep it up! 🔥':'Start writing today!';

  // Last edited
  const sorted=[...notes].sort((a,b)=>(b.updated||b.created||'').localeCompare(a.updated||a.created||''));
  const last=sorted[0];
  const leEl = document.getElementById('w-lastedited');
  const leTi = document.getElementById('w-lastedited-title');
  if(leEl) leEl.textContent = last?(last.updated||last.created||'').slice(0,10):'—';
  if(leTi) leTi.textContent = last?last.title:'No notes yet';
}
